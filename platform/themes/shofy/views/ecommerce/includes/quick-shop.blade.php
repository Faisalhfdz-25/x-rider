<div class="bb-product-detail">
    ...
    <div class="tp-product-details-price-wrapper mb-20">
        <span class="tp-product-details-price new-price" data-bb-value="product-price">{{ format_price($product->front_sale_price_with_taxes) }}</span>
        @if ($product->isOnSale())
            <span class="tp-product-details-price old-price" data-bb-value="product-original-price">{{ format_price($product->price_with_taxes) }}</span>
        @endif
    </div>
                ...
                @if (EcommerceHelper::isCartEnabled())
                    <div class="tp-product-details-add-to-cart w-100">
                        <button
                            ...
                            {!! EcommerceHelper::getGtmAttributes('add-to-cart-in-form', $product) !!}
                        >
                            {{ __('Add To Cart') }}
                        </button>
                    </div>
