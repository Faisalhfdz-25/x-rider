...
class InvoiceHelper
{
    ...
    {
        return (new Pdf())
            ->templatePath(plugin_path('ecommerce/resources/templates/invoice.tpl'))
            ...
            ->formatContentUsing(function (string $content): string {
                $currencies = [
                    '₼' => 'azeri-manat',
                    '₹' => 'indian-rupee',
                    '৳' => 'bangladeshi-taka',
                ...
                foreach ($currencies as $currency => $icon) {
                    $content = str_replace(
                        $currency,
                        Html::image(asset("vendor/core/plugins/ecommerce/images/currencies/$icon.svg"), 'currency', ['width' => 10, 'style' => 'margin-right: 2px;']),
                        $content
                    );
                }
